        ...
        for line_num, line in enumerate(lines):
            ...
            try:
                ...
                if len(parts) > 7 and not parts[0].isdigit():
                    try:
                        # Attempt to parse as date (adjust format as needed)
                        date_str = parts[0]
...
def create_enhanced_features(number_sets, window_size, is_main=True, dates=None):
    ...
    for i in range(len(number_sets) - window_size):
        ...
        if dates and dates[i] is not None:
            try:
                date = datetime.strptime(dates[i], "%Y-%m-%d")  # Adjust format as needed
        ...
        try:
            if model_type == "xgboost":
                ...
                model = lgb.LGBMRegressor(
                    n_estimators=100,  # Reduced for small datasets
                    learning_rate=0.1,
                    num_leaves=15,  # Reduced for small datasets
                    max_depth=3,  # Reduced for small datasets
                    subsample=0.8,
                    colsample_bytree=0.8,
                    reg_alpha=0.1,
                    reg_lambda=1.0,
                    min_child_samples=2,  # Minimum required for small datasets
                    random_state=42
                )
...
def advanced_ensemble_predict(all_predictions, model_evaluations, num_range, num_to_predict):
    ...
    for model_type in model_types:
        ...
        if mae_key in model_evaluations and model_evaluations[mae_key] > 0:
            ...
            weights[model_type] = 1.0 / model_evaluations[mae_key]
        else:
...
def main():
    ...
    # All model types
    all_model_types = traditional_model_types
    ...
    for model_type in all_model_types + ["ensemble"]:
        mae_key = f'{model_type}_mae'
